            ...
            switch (v.getId()) {
                case R.id.camera_vedio:
                    ...
                    if(isRecording){
                        ...
                        bool = false;
                        mediaRecorder.stop();
                        timeTextView.setText(Utils.format(hour)+":"+Utils.format(minute)+":"+ Utils.format(second));
                        mediaRecorder.release();
                        mediaRecorder = null;
                        Utils.videoRename(mRecAudioFile);
                        Log.e(tag, "=====录制完成，已保存=====");
                        isRecording = !isRecording;
                        try {
                            camera = Camera.open();
                ...
                case R.id.camera_photo:
                    if (mediaRecorder != null) {
                        try {
                            bool = false;
                            mediaRecorder.stop();
                            timeTextView.setText(Utils.format(hour) + ":" + Utils.format(minute) + ":"
                                    + Utils.format(second));
                            mediaRecorder.release();
                            mediaRecorder = null;
                            Utils.videoRename(mRecAudioFile);
